...
def test__add_task__to_tracker(task_tracker, new_task, database):
    assert task_tracker.get_all_tasks() is None
    task_tracker.add_task(new_task)
    assert new_task.id == IsUUID
    assert len(task_tracker.get_all_tasks()) == 1
...
def test__set_new_correct_status_to_task(task_tracker, new_task):
    ...
    updated_task = task_tracker.get_task_by_id(task_id)
...
def test__delete_task_by_id(task_tracker, new_task, database):
    task_id = task_tracker.add_task(new_task)
    assert len(task_tracker.get_all_tasks()) == 1
...
def test__get_all_tasks(task_tracker, database):
    ...
    assert len(task_tracker.get_all_tasks()) == 3
